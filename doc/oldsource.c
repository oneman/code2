...
write(1, "ok\n", 3);
  unsigned char *map = buf->map;
  for (int c = 0;c < 1920 * 1080 * 4;c++) {
    if (!(c % 3)) { map[c] = 0; continue; }
    map[c] = c * 26;
  }
...
int main1()
{
	...
	drm_setup_fb(fd, dev);
	for (;;) {
  for (int c = 0;c < dev->width * dev->height * 4;c++) {
    if (!(c % 3)) { dev->buf[c] = 1; continue; }
    dev->buf[c] = c * 26;
  }
	usleep(15);
	}
